...
selected_pipeline = st.selectbox(label="Which pipeline would you like?", options = pipelines, index=None, format_func=lambda artifact: artifact.name)
        ...
        if not dataset is None:
            ...
            if not target_feature is None and not input_features is None and not target_feature in input_features:
                if st.button(label="Run on new data"):
                    st.write(pipeline.evaluate_new_data(dataset, input_features, target_feature))
